 ...
 */
export function computeRiskMultiplier(snapshot) {
    let multiplier = 1.0;
 ...
 */
export function computeCategoryBias(snapshot) {
    const bias = {};
    for (const category of Object.keys(snapshot.categoryPnl30d)) {
        if (category === 'Other')
            continue;
 ...
 */
export function computePerformanceSnapshot(agentId, trades, startingCapital = 3000) {
    const now = Date.now();
    ...
    let runningPnL = 0;
    let peakPnL = 0;
    for (const trade of closedTrades.sort((a, b) => new Date(a.closedAt || a.openedAt).getTime() - new Date(b.closedAt || b.openedAt).getTime())) {
    ...
    const categoryPnl30d = {};
    const categoryTrades30d = {};
 ...
 */
export function computeAdaptiveConfig(agentId, trades, startingCapital = 3000) {
 ...
 */
export function getEffectiveRiskBudget(baseRisk, adaptiveConfig) {
 ...
 */
export function getCategoryWeightMultiplier(category, adaptiveConfig) {
    if (category === 'Other')
        return 1.0;
