 ...
 */
async function callAnthropic(context, webSearchResults) {
    ...
    const isRefusal = refusalIndicators.some(indicator => content.toLowerCase().includes(indicator.toLowerCase()));
    if (isRefusal) {
 ...
 * xAI GROK API (GROK 4)
 */
async function callGroq(context, webSearchResults) {
 ...
 */
async function callGoogleAI(context, webSearchResults) {
 ...
 */
async function callDeepSeek(context, webSearchResults) {
 ...
 * Qwen API (QWEN 2.5)
 */
async function callQwen(context, webSearchResults) {
 ...
 */
function buildTradePrompt(context, agentName, webSearchResults) {
...
function parseAIResponse(content) {
    try {
        ...
        const isRefusal = refusalIndicators.some(indicator => jsonStr.toLowerCase().includes(indicator.toLowerCase()));
        if (isRefusal) {
        ...
        const reasoning = Array.isArray(parsed.reasoning)
            ? parsed.reasoning.map((r) => String(r))
            : parsed.reasoning
 ...
 */
export async function getAITradeDecision(agentId, market, newsArticles, webSearchResults) {
 ...
 */
export function isAIConfigured(agentId) {
    switch (agentId) {
        case 'GPT_5':
