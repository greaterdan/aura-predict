...
/**
 * Personality rule result
 */
export interface PersonalityResult {
  side: TradeSide;
  confidence: number; // 0-1
  sizeUsd: number;
  notes: string[]; // Optional personality notes to surface in UI
}

/**
 * Personality rule function type
 */
export type PersonalityRule = (ctx: PersonalityContext) => PersonalityResult;
 ...
 */
export function grokMomentumRule(ctx: PersonalityContext): PersonalityResult {
  const { market, baseSide, baseConfidence, baseSizeUsd } = ctx;
  
  const isMomentumMarket =
    (market.category === 'Crypto' || market.category === 'Tech') &&
 ...
 */
export function claudeCrowdRule(ctx: PersonalityContext): PersonalityResult {
  const { market, baseSide, baseConfidence, baseSizeUsd } = ctx;
  
  const isCrowdedPolitical =
    (market.category === 'Politics' || market.category === 'Elections') &&
 ...
 */
export function geminiSportsRule(ctx: PersonalityContext): PersonalityResult {
  const { market, baseSide, baseConfidence, baseSizeUsd } = ctx;
 ...
 */
export function getPersonalityRules(agentId: string): PersonalityRule[] {
  const rules: Record<string, PersonalityRule[]> = {
